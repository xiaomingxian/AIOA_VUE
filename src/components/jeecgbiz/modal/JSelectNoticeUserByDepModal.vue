...
<script>
  ...
  export default {
    name: 'JSelectNoticeUserByDepModal',
    inject:['reload'],
    components: {},
    props: ['modalWidth'],
    data() {
    ...
    created() {
      ...
      // this.useridLists.split(',').map((item)=>{
    ...
    methods: {
      ...
      handleSubmit() {
        ...
        this.selectionRows.map((item,index)=>{
          ...
          this.chooseUserName+=item.username+','
        })
        ...
        this.$emit('ok', this.chooseUserName);
        this.$emit('getUserId',this.selectedRowKeys);

        // console.log(this.chooseUserName);
        // that.close();
