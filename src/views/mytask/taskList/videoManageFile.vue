  ...
  <center>
    ...
    <table border="1" class="sendFileStyle" style="border:#F0F5FC;" width="100%">
      ...
      <tr>
        ...
        <td colspan="3">
          <!--<a-textarea cols="92" rows="2" v-model="backData.i_bigint1"></a-textarea>v-model="select_type" -->
          <a-select style="width:100%" placeholder="请选择类别" ref="i_bigint1" v-on:blur="blurText(backData.i_bigint1,$refs.i_bigint1)"
      ...
      <tr>
        ...
        <td colspan="3">
          <div v-for="(item,index) in videoList" style="padding-left: 15px">
            <div class="qiCao"><span class="hoverred"@click="videoPlay(item.sfilePath)">{{index+1}}、{{item.sfileName}}</span>
...
<!--                <img :title="fileBtnName(1)" v-show ="isSuffex(item.sfileName)" class="pices" @click.stop="qiCao2(10,item)" src="../../../../src/assets/set.png"/>-->
                ...
                <img :title="fileBtnName(3)" class="pices" @click.stop="deleteFilesBtn(item,8)" src="../../../../src/assets/delete.png"/>
                <img :title="fileBtnName(4)" v-show="videoList.length > 1 && index > 0" class="pices" @click.stop="topFile(item,index,8)" src="../../../../src/assets/top.png"/>
                <img :title="fileBtnName(5)" v-show="videoList.length > 1 && index < videoList.length-1" class="pices" @click.stop="lowFile(item,index,8)" src="../../../../src/assets/bottom.png"/>
              </span>
            </div>
          </div>
        </td>
...
<script>
  ...
  import VideoPlayModal from "../../buttons/VideoPlayModal";

  export default {
    name: "VideoManageFile",
    ...
    methods: {
      videoPlay(address) {
        this.$refs.videoPlayModal.show(address);
